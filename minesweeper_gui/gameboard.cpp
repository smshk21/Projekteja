// Lisäsin kaksi viimeistä metodia.
// tämä tiedosto toteuttaa kaikki ei-graafisen pelilaudan operaatiot
// käyttäen square-luokkaa apuna.
// ---------------------------------------------------------------------
// Elias Keso / Norssi
// elias.keso@tuni.fi
// TUNI-tunnus: mrelke
...
bool GameBoard::liputOikein()
{
    for (auto& vektori : board_)
    {
        for (auto& ruutu : vektori)
        {
            if (ruutu.hasFlag() and not ruutu.hasMine())
            {
                return false;
            }
        }
